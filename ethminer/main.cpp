/*
    ...
    along with soft.  If not, see <http://www.gnu.org/licenses/>.
*/
...
bool g_exitOnError = false;  // Whether or not soft should exit on mining threads errors
...
#undef BACKTRACE_MAX_FRAMES
    ...
    {
        cout << "soft " << endl
             << "minimal usage : soft [DEVICES_TYPE] [OPTIONS] -P... [-P...]" << endl
             << endl
             << "Devices type options :" << endl
             << endl
             << "    By default soft will try to use all devices types" << endl
    ...
    void helpExt(std::string ctx)
    {
        ...
        if (ctx == "api")
        {
            cout << "API Interface Options :" << endl
                 << endl
                 << "    soft provide an interface for monitor and or control" << endl
        ...
        if (ctx == "misc")
        {
            cout << "Miscellaneous Options :" << endl
                 ...
                 << "                        Sets the number of minutes soft can stay" << endl
                 ...
                 << "    --exit              FLAG Stop soft whenever an error is encountered"
                 << endl
                 << "    --ergodicity        INT[0 .. 2] Default = 0" << endl
                 << "                        Sets how soft chooses the nonces segments to"
                 << endl
                 << "                        search on." << endl
        ...
        if (ctx == "con")
        {
            cout << "Connections specifications :" << endl
                 ...
                 << "    the fail-over disconnects soft passes to the next connection" << endl
                 << "    available and so on till the list is exhausted. At that moment" << endl
                 << "    soft restarts the connection cycle from the first one." << endl
                 << "    An exception to this behavior is ruled by the --failover-timeout" << endl
                 << "    command line argument. See 'soft -H misc' for details." << endl
                 << endl
                 << "    The special notation '-P exit' stops the failover loop." << endl
                 << "    When soft reaches this kind of connection it simply quits." << endl
                 << endl
                 << "    When using stratum mode soft tries to auto-detect the correct" << endl
    ...
    void doMiner()
    {
        ...
        if (m_mode != OperationMode::Simulation)
            for (auto conn : m_PoolSettings.connections)
                cnote << "Configuredha " << conn->Host() + ":" + to_string(conn->Port());
...
int main(int argc, char** argv)
{
    ...
    auto* bi = soft_get_buildinfo();
    cout << endl
         << endl
         << "soft " << bi->project_version << endl
    ...
    if (argc < 2)
    {
        cerr << "No arguments specified. " << endl
             << "Try 'soft --help' to get a list of arguments." << endl
             << endl;
        return 1;
    }
    ...
    try
    {
        ...
        catch (std::invalid_argument& ex1)
        {
            cerr << "Error: " << ex1.what() << endl
                 << "Try soft --help to get an explained list of arguments." << endl
                 << endl;
            return 1;
        }
